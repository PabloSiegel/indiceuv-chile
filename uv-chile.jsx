 ...
 * Routing: hash-based  →  #/region/{id_region}
 */

import { useState, useEffect, useCallback } from "react";

// ─── Configuración ────────────────────────────────────────────────────────────
// URL relativa: en dev Vite hace proxy a :3001; en prod Render usa el mismo servidor
const PROXY_URL = "/api/uv";

// Ciudades principales por id_region (la API solo devuelve nombre de región)
const CITY_MAP = {
  "1":  "Arica",
  "2":  "Iquique",
  "3":  "Antofagasta",
...
// ─── Helpers UV ───────────────────────────────────────────────────────────────
// Categorías según API: 1=Bajo 2=Moderado 3=Alto 4=MuyAlto 5=Extremo
function getUVInfo(uv, categoria) {
  const n = Number(uv);
  if (n <= 2 || categoria === "1") return {
    level: "Bajo",
    spf: "SPF 15",
    textColor: "text-green-400",
    accent: "from-green-400 to-green-500",
    pill: "bg-green-500/20 text-green-300 border-green-500/30",
    recommendation: "No se requiere protección especial. Disfruta el sol de forma segura.",
    reapply: "No necesario",
    hat: "Opcional",
  };
  if (n <= 5 || categoria === "2") return {
    level: "Moderado",
    spf: "SPF 30",
    textColor: "text-yellow-400",
    accent: "from-green-400 via-yellow-400 to-yellow-500",
    pill: "bg-yellow-500/20 text-yellow-300 border-yellow-500/30",
    recommendation: "Usa bloqueador solar y gafas de sol. Busca sombra al mediodía.",
    reapply: "Cada 2 horas",
    hat: "Recomendado",
  };
  if (n <= 7 || categoria === "3") return {
    level: "Alto",
    spf: "SPF 50+",
    textColor: "text-orange-400",
    accent: "from-green-400 via-yellow-400 to-orange-500",
    pill: "bg-orange-500/20 text-orange-300 border-orange-500/30",
    recommendation: "Usa bloqueador, lentes y evita el sol entre 11:00 y 16:00.",
    reapply: "Cada 2 horas",
    hat: "Recomendado",
  };
  if (n <= 10 || categoria === "4") return {
    level: "Muy Alto",
    spf: "SPF 50+",
    textColor: "text-red-400",
    accent: "from-yellow-400 via-orange-400 to-red-500",
    pill: "bg-red-500/20 text-red-300 border-red-500/30",
    recommendation: "Protección máxima. Evita exponerte entre 10:00 y 17:00.",
    reapply: "Cada hora",
    hat: "Obligatorio",
  };
  return {
    ...
    accent: "from-orange-400 via-red-500 to-violet-600",
    pill: "bg-violet-500/20 text-violet-300 border-violet-500/30",
    recommendation: "Peligro extremo. Permanece bajo techo entre 09:00 y 18:00.",
    reapply: "Cada hora",
    hat: "Obligatorio",
  };
}
...
function useRoute() {
  ...
  const [regionId, setRegionId] = useState(parse);
  useEffect(() => {
    const handler = () => setRegionId(parse());
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-12 text-center max-w-xs">
      ...
      <p className="text-slate-500 text-xs">
        Asegúrate de que{" "}
        <code className="bg-slate-800 px-1 py-0.5 rounded text-amber-400">node server.js</code>{" "}
        esté corriendo en el puerto 3001.
      </p>
      <button
        onClick={onRetry}
        className="mt-2 px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 text-sm font-semibold transition-colors"
      >
        Reintentar
      </button>
    </div>
...
function UVCard({ data, onBack }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* Ruta activa */}
      <div className="mb-3 text-xs text-slate-500 font-mono tracking-widest">
      ...
      <div className="w-[446px] h-[213px] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden font-sans">
        <div className="h-full grid grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="p-4 flex flex-col justify-between bg-gradient-to-br from-sky-50 via-white to-amber-50">
            <div>
              <div className="text-[11px] tracking-[0.18em] uppercase text-slate-500 font-semibold">
                Índice UV
              </div>
              <h1 className="text-[22px] font-bold text-slate-900 leading-none mt-1">
                {city}
              </h1>
            ...
            <div>
              <div className="flex items-end gap-2">
                ...
                <span className={`text-[15px] font-semibold mb-2 ${info.textColor}`}>
                  {info.level}
                </span>
              </div>
              <p className="text-[11px] text-slate-600 mt-1 leading-snug max-w-[210px]">
                {info.recommendation}
              </p>
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-950 text-white flex flex-col justify-between">
            <div className="flex items-center justify-between">
              ...
              <div className="w-11 h-11 rounded-full bg-gradient-to-br from-yellow-300 to-orange-500 flex items-center justify-center text-[20px] shadow-md">
                ☀️
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="w-full h-3 rounded-full bg-slate-800 overflow-hidden">
                <div
                  className={`h-full rounded-full bg-gradient-to-r ${info.accent} transition-all duration-700`}
      ...
      <p className="mt-3 text-[10px] text-slate-600">
        Fuente: indiceuv.cl · Actualizado hoy
      </p>
    </div>
  );
}
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="mb-6 text-center">
        <div className="text-[11px] tracking-[0.22em] uppercase text-slate-500 font-semibold mb-1">
          Chile · Índice UV
        </div>
        <h1 className="text-2xl font-black text-white">Selecciona tu región</h1>
        {dateMessage && (
          <p className="text-slate-400 text-xs mt-1">{dateMessage}</p>
        )}
      </div>
      ...
      <div className="space-y-2">
        {regions.map((r) => {
          ...
          return (
            <button
              key={r.id_region}
              onClick={() => onSelect(r.id_region)}
              className="w-full flex items-center gap-3 bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-slate-600 rounded-xl px-4 py-3 transition-all group"
            >
              {/* Número UV */}
              ...
              <div className="flex-1 text-left min-w-0">
                ...
                <div className="text-[11px] text-slate-500">
                  {CITY_MAP[r.id_region] ?? ""}
                </div>
                {/* Mini barra */}
                <div className="w-full h-1.5 rounded-full bg-slate-800 mt-1.5 overflow-hidden">
                  <div
                    className={`h-full rounded-full bg-gradient-to-r ${info.accent} transition-all duration-500`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
              ...
              {/* Badge nivel */}
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border flex-shrink-0 ${info.pill}`}>
                {info.level}
              </span>
      ...
      <button
        onClick={onRefresh}
        className="mt-4 w-full text-xs text-slate-600 hover:text-slate-400 transition-colors flex items-center justify-center gap-1.5 py-2"
      >
...
export default function UVIndiceApp() {
  const { regionId, navigate } = useRoute();
  ...
  // Carga todas las regiones al iniciar
  ...
  // Región activa (buscada en el cache)
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center py-10 px-4">
      {loading ? (
        ...
        // Región en URL pero no cargada aún (raro, pero posible)
